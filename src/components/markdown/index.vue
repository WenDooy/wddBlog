   ...
   <div class="markdown-content" :style="{background:preview==2?'#fff':''}">
     <div v-show="preview===1||preview===3" class="markdown-editor" ref="markdownContent"
       ...
       <ul class="index" ref="index" :style="{height:scrollHeight?`${scrollHeight}px`:'100%'}">
         <li v-for="(item,index) in indexLenth" v-bind:key="index">{{index+1}}</li>
       </ul>
       <textarea
         v-model="value"
         @keydown.tab="tab"
...
<style lang="less">
  ...
  @media (max-width: 700px) {
    .icon-checked-false:before, .icon-checked:before, .icon-link:before,
    .icon-img:before, .icon-table:before, .icon-daoru:before, .icon-download:before,
    .icon-md:before, .icon-group:before, .icon-preview:before, .icon-on:before,
    .icon-off:before, .icon-yanse:before{
      content: "";
    }
  }
</style>
